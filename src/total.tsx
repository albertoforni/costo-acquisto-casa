...
export function Total() {
  ...
  return (
    <section class="bg-card rounded-lg border shadow-sm p-6 space-y-6">
      <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-card-foreground flex items-center gap-2">
          <Calculator class="h-6 w-6" />
          Totali
        </h2>
        <div class="h-px flex-1 mx-4 bg-gradient-to-r from-transparent via-border to-transparent"></div>
      </div>

      <div class="space-y-6">
        <div class="grid grid-cols-3 gap-4 items-center">
          <label
            class="col-span-2 text-lg font-semibold text-card-foreground"
            for="totalCost"
          >
            Totale costi
          </label>
      ...
      <div class="mt-4 p-4 bg-muted rounded-md border border-border">
        <p class="text-sm text-muted-foreground flex items-start gap-2">
          <Info class="h-4 w-4 mt-0.5 flex-shrink-0" />
          <span>
            Il totale esborso monetario include il prezzo di acquisto più tutti
            i costi accessori. I costi accessori includono agenzia, tasse, mutuo
            e notaio.
          </span>
        </p>
      </div>
